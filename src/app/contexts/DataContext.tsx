...
interface DataContextValue {
  ...
  addProgramme: (name: string, departmentId: string) => Promise<Programme>;
...
export const DataProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  // ✅ Load protected data based on role after login
  useEffect(() => {
  ...
  const addProgramme = useCallback(async (name: string, departmentId: string): Promise<Programme> => {
  ...
  const updateProgramme = useCallback(async (id: string, name: string, departmentId: string) => {
  ...
  const getAttendanceSummary = useCallback((studentUserId: string) => {
    ...
    return (student?.enrolledCourses ?? []).map(courseId => {
      ...
      // ✅ Late counts as 0.5 towards attendance percentage
