...
export default function CoursesPage() {
  ...
  const handleSave = () => {
    ...
    if (modal === "add") {
      ...
      // Preserve lecturerId when editing — admin doesn't change it here
  ...
  // Group displayed courses by department then year for a nicer view
    ...
    <div className="space-y-5 p-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select value={deptFilter} onChange={e => setDeptFilter(e.target.value)}
          className="border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-600">
      ...
      {/* Course list grouped by dept → year */}
      {filtered.length === 0 ? (
        ...
        <div className="space-y-6">
          {Object.entries(groupedCourses).map(([deptId, { deptName, byYear }]) => (
            <div key={deptId}>
              <h2 className="text-sm font-semibold text-gray-700 mb-3 pb-1 border-b border-gray-200">{deptName}</h2>
              <div className="space-y-4">
                {Object.keys(byYear).sort().map(yearKey => (
                  <div key={yearKey}>
                    <p className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-2">Year {yearKey}</p>
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                      {byYear[Number(yearKey)].map(course => {
                        ...
                        return (
                          <div key={course.id} className="bg-white rounded-xl border border-gray-200 p-4 hover:border-blue-300 transition-colors">
                            ...
                            <p className="font-medium text-gray-900 text-sm leading-snug">{course.name}</p>
                            {course.description && (
                            ...
                            <div className="mt-3 pt-3 border-t border-gray-100 space-y-1">
                              ...
                              <div className="flex items-center gap-1 text-xs text-gray-400">
                                <User size={11} />
                                {lecturer
                                  ? <span className="text-indigo-600 font-medium">{lecturer}</span>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              <h2 className="font-semibold text-gray-900">{modal === "add" ? "Add Course" : "Edit Course"}</h2>
            ...
            <div className="p-5 pt-0 flex gap-3">
              ...
              <button onClick={handleSave} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white rounded-lg py-2 text-sm font-medium">
